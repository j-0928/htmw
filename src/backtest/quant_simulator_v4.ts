...
export interface QuantResult {
    ...
    meanBranch: number;  // Averaged expected return
...
export class MonteCarloEngine {
    ...
    static runSimulation(candles: any[], steps = 390, paths = 1000): QuantResult | null {
...
if (isMainThread) {
    if (process.argv[1]?.includes('quant_simulator_v4')) {
        ...
        function finalize(results: QuantResult[]) {
            ...
            console.table(results.slice(0, 40).map(r => ({
                ...
                'Mean Branch': r.meanBranch.toFixed(2) + '%',
